...
public class ThreeInOne {
    ...
    /*
     * Time Complexity of each stack operation is O(1)
     * Space Complexity O(n)
     */
    static class ThreeStack<T>
    {
        T arr[]; //store any type array
        int top[]; // store indexes of top elements in stack
        int next[]; //next entry and free list

        int n,k;
        int free; //beginning index of the free list

        ThreeStack(int numOfStacks, int n1)
        {
            k = numOfStacks;
            n = n1;
            arr = (T[])new Object[n];
        ...
        void push(T item, int stackChoice) throws Exception
        {
            //check overflow
            if(isFull())
            {
                throw new Exception("The chosen stack is full");
            ...
            //update to next free slot
            free = next[index];

            //update top and next of top
            next[index] = top[stackChoice];
            top[stackChoice] = index;
       ...
       T pop(int stackChoice) throws Exception
        {
            //check underflow
            if(isEmpty(stackChoice))
            {
            ...
            //find index of top item
            int index = top[stackChoice];

            //change top to store previous top
            top[stackChoice] = next[index];

            //attach previous top to beginning of free list
            next[index] = free;
            free = index;
        ...
        boolean isEmpty(int stackChoice)
        {
            return top[stackChoice] == -1;
        }
    ...
    /*
     * Book Approach
     * Time complexity O(n) for array but operations are O(1)
     * Space Complexity O(n)
     */
